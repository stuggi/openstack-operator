...
func (r *OpenStackControlPlaneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			corev1beta1.OpenStackControlPlaneBackupConfigReadyCondition,
			condition.ErrorReason,
			condition.SeverityWarning,
			corev1beta1.OpenStackControlPlaneBackupConfigReadyErrorMessage,
			err.Error()))
		return ctrl.Result{}, err
	}
	instance.Status.Conditions.MarkTrue(corev1beta1.OpenStackControlPlaneBackupConfigReadyCondition, corev1beta1.OpenStackControlPlaneBackupConfigReadyMessage)
...
func (r *OpenStackControlPlaneReconciler) SetupWithManager(
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		// Watch cert secrets for backup annotation changes. When a user adds
		// annotation overrides (e.g., backup.openstack.org/restore=true) to a
		// cert secret, the controller reconciles so EnsureCert can read the
		// annotation and update the Certificate CR's SecretTemplate labels,
		// preventing cert-manager from overwriting the user's override.
		Watches(
			&corev1.Secret{},
