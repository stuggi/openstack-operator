...
// gvkFromCRD extracts the GVK from a CRD, preferring the storage version.
func gvkFromCRD(crd *apiextensionsv1.CustomResourceDefinition) schema.GroupVersionKind {
	var version string
...
// Resources with ownerReferences are skipped (operator-managed).
// Resources that already have a restore label (set by operators at creation time)
// are skipped to preserve operator-set values.
// Annotation overrides on a resource bypass both checks, and changed annotation
// values are always re-synced to labels via EnsureBackupLabels.
...
func (r *OpenStackBackupConfigReconciler) labelCRInstances(ctx context.Context, log logr.Logger, instance *backupv1beta1.OpenStackBackupConfig) (int, error) {
	// Fallback: build cache if SetupWithManager failed to populate it.
	...
	for crdName, backupConfig := range r.CRDLabelCache {
		...
		if err != nil {
			...
			errs = append(errs, fmt.Errorf("CRD %s: %w", crdName, err))
			continue
		}
		...
		if err := r.List(ctx, list, client.InNamespace(instance.Namespace)); err != nil {
			...
			errs = append(errs, fmt.Errorf("list %s: %w", crdName, err))
			continue
		}
		...
		for i := range list.Items {
			...
			if _, err := backup.EnsureBackupLabels(ctx, r.Client, obj, defaultLabels); err != nil {
				...
				errs = append(errs, fmt.Errorf("%s/%s: %w", gvk.Kind, obj.GetName(), err))
				continue
			}
			count++
		}
...
func (r *OpenStackBackupConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if err != nil {
		...
		instance.Status.Conditions.Set(condition.TrueCondition(
			backupv1beta1.OpenStackBackupConfigSecretsReadyCondition,
			"%d secrets have backup labels", secretCount))
	}
	...
	if err != nil {
		...
		instance.Status.Conditions.Set(condition.TrueCondition(
			backupv1beta1.OpenStackBackupConfigConfigMapsReadyCondition,
			"%d configmaps have backup labels", configMapCount))
	}
	...
	if err != nil {
		...
		instance.Status.Conditions.Set(condition.TrueCondition(
			backupv1beta1.OpenStackBackupConfigNADsReadyCondition,
			"%d NADs have backup labels", nadCount))
	}
	...
	if err != nil {
		...
		instance.Status.Conditions.Set(condition.TrueCondition(
			backupv1beta1.OpenStackBackupConfigCRsReadyCondition,
			"%d CRs have backup labels", crCount))
	}
	...
	instance.Status.LabeledResources.CRs = crCount
...
func (r *OpenStackBackupConfigReconciler) findBackupConfigForSrc(ctx context.Context, obj client.Object) []reconcile.Request {
	configList := &backupv1beta1.OpenStackBackupConfigList{}
	if err := r.List(ctx, configList, client.InNamespace(obj.GetNamespace())); err != nil {
...
func (r *OpenStackBackupConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	bldr := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&corev1.Secret{}, handler.EnqueueRequestsFromMapFunc(r.findBackupConfigForSrc), builder.WithPredicates(backupResourcePredicate)).
		Watches(&corev1.ConfigMap{}, handler.EnqueueRequestsFromMapFunc(r.findBackupConfigForSrc), builder.WithPredicates(backupResourcePredicate)).
		Watches(&k8s_networkingv1.NetworkAttachmentDefinition{}, handler.EnqueueRequestsFromMapFunc(r.findBackupConfigForSrc), builder.WithPredicates(backupResourcePredicate))
	...
	if err != nil {
		...
		for crdName := range cache {
			...
			bldr = bldr.Watches(obj, handler.EnqueueRequestsFromMapFunc(r.findBackupConfigForSrc), builder.WithPredicates(backupResourcePredicate))
