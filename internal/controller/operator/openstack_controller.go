...
func (r *OpenStackReconciler) applyManifests(ctx context.Context, instance *operatorv1beta1.OpenStack) error {
	Log := r.GetLogger(ctx)
	// only apply CRDs and services once per each containerImage change
	if !containerImageMatch(instance) {
		...
		if err := r.applyServices(ctx, instance); err != nil {
			Log.Error(err, "failed applying Services manifests")
			return err
		}
	}
...
func (r *OpenStackReconciler) applyServices(ctx context.Context, instance *operatorv1beta1.OpenStack) error {
	data := bindata.MakeRenderData()
	data.Data["OperatorNamespace"] = instance.Namespace
	return r.renderAndApply(ctx, instance, data, "services", true)
}
