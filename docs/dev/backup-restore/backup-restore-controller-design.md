...
- ❌ **Operator-created leaf cert secrets** (Certificate CR has ownerRef, `spec.isCA != true`) — not labeled for restore; cert-manager regenerates them from the restored CA Issuer
...
- Operator-created leaf cert Secrets (cert-manager) → Not labeled → Not restored, regenerated by cert-manager ✅
...
*All Secrets/ConfigMaps included in full namespace backup; controller labels user-provided ones (no ownerReferences) for restore, plus CA cert secrets (cert-manager `isCA: true`); operator-managed leaf cert secrets are excluded from restore (regenerated by cert-manager)
